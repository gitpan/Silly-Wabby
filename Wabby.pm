...
# Constructor.  Note that Silly::Wabby only supports an OO interface.
...
Silly::Wabby - Create semi-random sentences based upon a body of text.

=head1 SYNOPSIS

  use Silly::Wabby qw(:errors);

  # Use the default options
  my $wabby = Silly::Wabby->new;

  # Pass in explicit options. (All options below are defaults)
  my $wabby = Silly::Wabby->new( min_len => 3, max_len => 30,
...
Silly::Wabby only provides an object-oriented interface, and exports no
...
To begin using Silly::Wabby you must first create a new object:
...
Silly::Wabby can save and load state to disk using the Storable module.  To do
...
=head1 CREDITS
 ...
 Project Gutenberg - for providing free text to feed to Silly::Wabby.
